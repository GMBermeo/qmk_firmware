...
[_NUMP] = LAYOUT_ortho_4x12_1x2uC(
    ...
    QK_RESET, _______, _______, _______, TG(3),    TG(3),  _______,  TG(3),     KC_P0,   _______, _______
...
bool process_record_user(uint16_t keycode, keyrecord_t* record) {
    switch (keycode) {
        // ç key
        case CEDILHA:
            return process_key_with_mods(KC_C, KC_QUOT, record);
        // ã key
        case AO:
            return process_key_with_mods(KC_A, KC_LSFT, record);
        // direct " key
        case ASPAS:
            return process_key_with_mods(KC_SPC, KC_LSFT, record);
